import { normalize, subtract, dot, cross, distance } from "../world";
import { Controls } from "./controls";
import { NeuralNetwork } from "./network";
import { NNEditor } from "./new_nn/nnEditor";
...
export class Car {
    ...
    carImg: any;
    boatImg: HTMLImageElement;
    helicopterImg: HTMLImageElement;
    mask: HTMLCanvasElement;
    carMask: any;
    boatMask: HTMLCanvasElement;
    helicopterMask: HTMLCanvasElement;
    tm: NodeJS.Timeout;
    target: any;
    layer: number;
    polygon: any[];
    borderDamaged: boolean;
    inputs: any;
    invulnerable: any;
    absDist: number;
    parking: any;
    ...
    update(
        ...
        if (!this.damaged) {
            ...
            if (this.sensor) {
                ...
                if (this.brainOptions.extraInputs.includes("â±ï¸")) {
                    offsets.push(this.speed / this.maxSpeed);
                }
                if (this.brainOptions.extraInputs.includes("ðŸ›‘")) {
                    offsets.push(Math.max(...stopOffsets));
                }
                if (this.brainOptions.extraInputs.includes("ðŸš¦")) {
                    offsets.push(Math.max(...lightOffsets));
                }
                if (this.brainOptions.extraInputs.includes("ðŸš¶")) {
                    offsets.push(Math.max(...crossingOffsets));
                }
                if (this.brainOptions.extraInputs.includes("âš ï¸")) {
                    offsets.push(Math.max(...yieldOffsets));
                }

                if (this.brainOptions.extraInputs.includes("ðŸŽ¯")) {
                    if (target) {
    ...
    private assessDamage(roadBorders, carBorders) {
