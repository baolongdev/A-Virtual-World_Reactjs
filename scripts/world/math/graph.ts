...
import { dot, getNearestSegment, normalize, subtract } from "./utils";
// TODO Config
...
export class Graph {
    ...
    getSegmentsLeavingFromPoint(point: Point) {
        const segs: Segment[] = [];
        for (const seg of this.segments) {
            if (seg.oneWay) {
    ...
    shortestPath(car, endPoint) {
        ...
        while (!end.visited) {
            ...
            for (const seg of segs) {
                const ep: Point = seg.p1.equals(currentPoint) ? seg.p2 : seg.p1;
                //if (!ep.visited) {
    ...
    shortestPath_SLOW(car, endPoint) {
        ...
        while (!end.visited) {
            ...
            for (const seg of segs) {
                const ep: Point = seg.p1.equals(point) ? seg.p2 : seg.p1;
